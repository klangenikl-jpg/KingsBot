...
void OnTick()
{
   ...
   if(g_swing.Evaluate(_Symbol,InpTimeframe,s))
      signals[count++]=s;

   if(g_structure.Evaluate(_Symbol,InpTimeframe,s))
      signals[count++]=s;

   if(g_liquidity.Evaluate(_Symbol,InpTimeframe,s))
      signals[count++]=s;

   if(g_orderblock.Evaluate(_Symbol,InpTimeframe,s))
      signals[count++]=s;

   if(g_fvg.Evaluate(_Symbol,InpTimeframe,s))
      signals[count++]=s;
